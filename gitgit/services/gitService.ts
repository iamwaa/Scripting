...
/** 对比页提交列表每侧最多加载条数，计数超过上限时同样截断 */
const COMPARE_COMMITS_LIMIT = 200

/**
 * 沿两侧第一父交替查找最近交点，优先处理 fork 常见的线性历史。
 * 避免大仓库一开始就调用开销较高的完整 findMergeBase。
 */
...
/** 从 tip 沿第一父收集领先或落后提交，最多 limit 条。 */
async function collectCompareCommits(
  ...
  while (current && current !== stopOid && entries.length < limit) {
    ...
    entries.push({
      ...
      date: new Date(
        (result.commit.author?.timestamp || 0) * 1000
      ).toISOString(),
    })
...
/**
 * origin 当前分支与「远端管理」中设置的目标分支差异。
 * 任一远端跟踪 ref 缺失时返回 null，由 UI 显示空态。
 * 计算领先/落后并返回对应提交列表；不读取文件树或文件改动历史。
 * ctx 仅供测试注入；生产环境不传，内部走 getCtx。
 */
async function compareWithUpstreamInternal(
  ...
  // 远端管理中的 upstream 是对比目标；未设置时回退 origin/当前分支。
  ...
  const baseTrack = `origin/${baseBranch}`
      ...
      try {
        const bases = (await withCompareTimeout(
          git.findMergeBase({
            ...
            dir,
            gitdir,
            oids: [baseOid, targetOid],
          })
        )) as string[]
        mergeBaseOid = bases?.[0] || null
        checkRemoteCancelled(options)
      } catch (e: any) {
        if (String(e?.message || e).includes("对比超时")) throw e
        mergeBaseOid = null
      }
    }
  }

  // 统计数量并收集领先、落后提交；不读取文件树。
