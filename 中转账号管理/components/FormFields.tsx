import { useState, Text, TextField, HStack, Button, Image } from "scripting"
...
export function LabeledTextField({ title, value, onChanged, prompt, axis }: { title: string, value: string, onChanged: (value: string) => void, prompt?: string, axis?: "horizontal" | "vertical" }) {
  ...
  return <HStack spacing={12}>
    ...
    <TextField title="" value={value} onChanged={onChanged} prompt={prompt} axis={axis as any} frame={{ maxWidth: "infinity" }} keyboardType="default" onFocus={() => setFocused(true)} onBlur={() => setFocused(false)} />
